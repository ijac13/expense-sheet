...
test("AC-11: the report path is gitignored and an analysis run leaves the tree clean", async () => {
  // Same gitignored directory as DEFAULT_REPORT, but never its filename — writing
  // the real report path would clobber an operator's actual candidate report.
  const probe = path.join(path.dirname(DEFAULT_REPORT), `gitignore-probe-${process.pid}.md`);
  fs.mkdirSync(path.dirname(probe), { recursive: true });
  fs.writeFileSync(probe, "# generated by a test\n", "utf8");
