/**
 * Entity 065 — backfill the 3 real recurring expenses (Wei 勞保, 三人健保, Uber 公會會費)
 * for February-August 2026, the months the captain paid them before her 2026-09-07
 * Subscriptions setup began covering them going forward.
 *
 * The 3 subscriptions' amount/category/payer and the 7-month window are pinned by
 * the approved ideation/spec — a literal list here, never read from the live
 * Subscriptions tab or derived from `now` — so a later edit to a subscription's
 * current amount, or a run any month after August 2026, cannot silently change
 * what this backfill writes for Feb-Aug 2026.
 *
 * Usage — every phase needs an explicit --target:
 *   node -r ./scripts/load-local-env.js scripts/backfill-subscription-065.js --target staging --dry-run
 *   node -r ./scripts/load-local-env.js scripts/backfill-subscription-065.js --target staging --apply
 *   node -r ./scripts/load-local-env.js scripts/backfill-subscription-065.js --target staging --undo
 *
 ...
 *
 * Write mechanism reused unmodified from entity 051: `buildColumnMap`/`buildWriteRow`
...
const ID_PREFIX = "exp-sub065-";
const REPORT_DIR = path.resolve(__dirname, "..", "backfill-reports");

// Pinned by the approved ideation/spec — never read from the live Subscriptions
// tab (AC-3's falsifier: reading "current" subscription state would let a later
// edit rewrite Feb-Aug 2026 history on a re-run).
const SUBSCRIPTIONS = [
  { subscriptionId: "sub-1788758970594", amount: 2105, category_id: "cat_024", paid_by: "wei", created_by: "wei", notes: "勞保" },
  { subscriptionId: "sub-1788758990082", amount: 2745, category_id: "cat_024", paid_by: "wei", created_by: "wei", notes: "三人健保" },
  { subscriptionId: "sub-1788759015607", amount: 150, category_id: "cat_006", paid_by: "wei", created_by: "wei", notes: "" },
];
...
const REQUIRED_CATEGORIES = ["cat_024", "cat_006"];
...
// "150" (existing row) and 150 (candidate) must compare equal.
 ...
 * live state, not a manifest — so the pre-existing Uber/Feb row is caught the same
 * way on the first run or the tenth (AC-2, AC-8).
 */
...
/** Aborts before any write if either required category is missing (AC-7). */
...
// Same all-or-nothing shape the scheduler and entity 051 use: the insert and the
// write ride in one batchUpdate, so a rejected write never leaves a blank row.
...
function manifestPathFor(target, override) {
  return override ?? path.join(REPORT_DIR, `065-manifest-${target}.json`);
}
...
async function run(argv, { log = console.log, env = process.env, sheetsFor = sheetsClientFor, now = () => new Date() } = {}) {
  ...
  const manifestPath = manifestPathFor(args.target, args.manifest);
  let manifest = { entity: "065", target: args.target, spreadsheetId: pair.spreadsheetId, writtenIds: [], rows: [] };
  if (fs.existsSync(manifestPath)) {
    try {
  ...
  manifest = { entity: "065", target: args.target, spreadsheetId: pair.spreadsheetId, at: createdAtIso, writtenIds: [...known], rows };
