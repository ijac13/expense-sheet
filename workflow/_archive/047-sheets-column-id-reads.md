...
**Production deploy:** `firebase deploy --only functions --project production` run 2026-08-16 (functions-only — this entity touched no `app/` code). Confirmed live: `GET /api/categories` on production returns `cat_001` with `gov_category:"restaurants_accommodation"` and `note:""` — both resolved correctly through the new header-based code.
