...
- DONE: Re-run the full test suite fresh (npm install/ci, not symlinks) in both app/ and functions/ and confirm 118/118 and 160/160 respectively
  `node_modules` were real directories, not symlinks; ran `npm ci` (full reinstall) in both first. app 118/118, functions 160/160, both exit 0. `npm run build` in `app/` clean — and its route list (`/`, `/history`, `/reports`, `/settings*`, `/subscriptions`, `/expense/[id]`, `/sign-in`) contains no migration route, which is live-ish proof of AC-5.
- DONE: Independently reproduce the build's falsification claims … Confirm exact counts, not approximate.
  Both exact. Reverting `TodayExpenseList.tsx` to main's version: **6 of 7** AC-13 tests fail, test 1 with literally `actual: 'cat_001'` / `expected: 'Eating Out'`; the lone survivor is the empty-state test, which never resolves a category. Stubbing `resolveCategory`'s live-id branch to `const live: Category | undefined = undefined`: **9 of 14** fail, and the 5 survivors are exactly the named set — fixture guard (t7), orphan fallback (t11), slug bridge (t12), offline (t13), empty state (t14). Both mutations reverted; `git status` clean.
- DONE: Independently verify the core migration script logic against a fixture … AC-1/2, AC-3, AC-7, AC-8, AC-9
  Wrote my own harness (fake Sheets client built from scratch, **not** `functions/test/sheetsStub.js`) driving the exported `run()`. 20/20 checks pass. AC-3 is the load-bearing one: my live Categories fixture uses `cat_901`–`cat_905`, so a script carrying a hardcoded `eating-out→cat_001` table would produce `cat_001`; it produced `cat_901`, i.e. the mapping genuinely comes from `resolveCategory()` against the live list. AC-7 halted with zero writes and a byte-identical grid on all five unmappable shapes (absent slug, absent `cat_NNN`, renamed twin, untrimmed whitespace, duplicate `name_en`) in **both** live and dry-run mode, and one halt names values from both tabs with row counts. AC-8 re-run: 0 changes, 0 `batchUpdate` calls. AC-9/AC-10: every non-`category_id` cell byte-identical, Categories untouched. AC-11: a tab with `category_id` moved to column B was written at `B`, never `D`.
- DONE: Confirm --dry-run structurally cannot write: verify it mints only the spreadsheets.readonly scope
  Proved at the real CLI entrypoint, not by reading the constant: intercepted `googleapis` in the require cache and ran `scripts/normalize-category-ids.js --dry-run`, which minted exactly `["…/auth/spreadsheets.readonly"]`; the same script with no flag minted `["…/auth/spreadsheets"]`. So the flag selects the credential, not merely a branch. **Bonus falsification:** because my fake accepted `batchUpdate` without persisting, the live run's post-run verification caught it — `Expenses!D2: expected "cat_901", found "eating-out"`, exit 1. The verification step is load-bearing, not a rubber stamp.
- DONE: Independently confirm the TodayExpenseList dead-code finding
  Accurate. Repo-wide grep (all `.ts/.tsx/.js/.jsx`, excluding `node_modules`/`.next`/build output) finds exactly two non-workflow hits: its own `export default` and its new test file. Zero imports. `app/app/page.tsx` imports `CategoryPicker` and `DatePickerModal` only, and line 139 renders `{expenses.length} {t("home.logged_today")}` — a count, no per-expense list. The build report's claim is correct as stated.
- DONE: Confirm scope boundary
  9 files, all in scope: the AC-13 component, 2 new app test files, `app/test/helpers/dom.js`, 2 `package.json` script blocks, the migration script, its test, `functions/test/sheetsStub.js`. `app/app/lib/categories.ts` is **untouched** — `resolveCategory` still exported, `DEFAULT_CATEGORIES` 24 entries, `CATEGORY_ICONS` 24 keys, and the bridge still resolves `eating-out` to a live twin. No `functions/src/index.ts` change, so no API/route/scheduler change. The two shared test-infra edits are additive and *stricter*, not weaker: `sheetsStub.js` extracts the existing over-wide-write 400 guard into `writeRange` and reuses it for the new `batchUpdate`, and `dom.js` adds an `expenses` fixture param defaulting to the existing `EXPENSES` while returning per-test copies.
- DONE: Live evidence — plain HTTP check confirming staging/production still serve correctly
  Staging root `HTTP 200` (11137 B), production root `HTTP 200` (10702 B); `/api/categories` serves live JSON on both. Went further and re-validated the spec's Live Baseline preconditions (traced 2026-08-21, now 3 days stale) against the production API: **25** live categories `cat_001`–`cat_025`, zero duplicate `name_en`, zero untrimmed values, zero inactive, and every `DEFAULT_CATEGORIES` slug has an exact live twin — so **zero unmappable values, and the production run will not halt**. Confirms AC-7 is a guard, not an expected branch, as of today.
- DONE: Mandatory PII/secrets check on the full diff
  Clean. No `.env` or credential files; no key/token/private-key/password-shaped strings; no email addresses; no phone numbers; no spreadsheet ids. The only URLs added are the two public Google OAuth scope constants. Test fixtures use `paid_by`/`created_by` of `"Karen"` — a bare first name with no contact data, already the established fixture convention on `main` (8 pre-existing occurrences in `app/test/helpers/dom.js` and `app/test/subscription-dates.render.test.js`), so this is the same bar prior entities passed. Real user identities stay out of the repo by design: `app/app/lib/users.ts` sources emails from env vars.
- DONE: If everything holds, set verdict PASSED. Note whether the TodayExpenseList wire-up-vs-delete question needs to go to the captain
  Verdict PASSED. **Yes — it needs the captain, and I am not resolving it.** Flagged below as gate finding 1.

### Summary

Everything the build claimed holds under independent re-derivation, including both falsification counts to the exact test. The strongest evidence is not the passing suite but two mutations: swapping in a live Categories fixture with non-production ids (`cat_901`) proves AC-3's mapping really is derived from `resolveCategory()` rather than a hardcoded table, and intercepting `googleapis` proves `--dry-run` mints a readonly credential — a write is impossible for the token, not merely unreached by the code. A third result was unplanned: my fake accepted writes without persisting them, and the script's post-run verification caught the discrepancy and exited non-zero, which demonstrates that verification step is real.

Three findings for the captain, none of them blockers:

1. **`TodayExpenseList` wire-up-vs-delete (needs a captain decision).** The AC-13 fix is correct and I verified it, but the component has zero imports repo-wide, so it repairs a surface no user currently sees. AC-13 and Surfaced Gap 2 both assert it is "visible on the home screen immediately after the run" — that assertion is false, and entity 049 recorded the same conclusion independently. Either wire it into Home (which is what the spec's language assumes) or delete it along with `CATEGORY_ICONS`. Not a migration-entity call.
2. **Staging would halt; production would not.** Staging's Categories tab has no `Insurance` or `Tax` twin (entity 042 never landed there — it is blocked on production sheets credentials), so running this script against staging halts under AC-7 with zero writes. That is the guard working correctly, and staging is explicitly out of scope, but the captain should not read that halt as a defect if anyone tries it.
3. **Entity 058 should land before or alongside the production run**, as the spec already recommends — `getDefaultCategory()` still returns the `eating-out` slug and actively rejects a stored live id, so legacy slugs begin re-accumulating the moment the migration finishes.

No production or staging data was written during this stage. The actual migration run remains a separate captain-gated step.
