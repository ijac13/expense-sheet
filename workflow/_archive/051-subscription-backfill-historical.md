...
- `functions/scripts/fixtures/backfill-sample/Subscriptions.json:26` `0928110757`, `:37` `0981811423`
- `functions/scripts/fixtures/backfill-sample/Expenses.json:249` `電信 0981811423`
...
- `Subscriptions.json:48,59,70,81` — `ijac 健身`, `Wei 健身`, `公公健身` (father-in-law), `婆婆健身` (mother-in-law), all at 788
...
- DONE: Two Taiwan mobile-number-format subscription names (0928110757, 0981811423) … replace with obviously-synthetic phone-format ids … that preserve the same amount+category collision with the YouTube-shaped fixture and the notes-vs-amount CONFLICTED shape
...
- DONE (blocked at cycle-2 tip, resolved by history squash — see Addendum): re-run the mandatory PII/secrets sweep … across every tracked file this fix touches (git diff, not just the files you think you changed) and confirm zero matches for any real phone number, real family-member subscription name, or real production amount
  At cycle-2 tip, all four touched files were clean **at HEAD**, but the sweep over `git diff origin/main...HEAD` still returned every original value, because build commit `229f751` was an ancestor of HEAD and its blobs carried the PII regardless of the tip. Verified: `git log -S'0928110757' -- functions/` returned `229f751`, and `git merge-base --is-ancestor 229f751 HEAD` succeeded. A rename alone could not clear this. The FO squashed the branch's history post-cycle-2 (see Addendum) — `229f751` is no longer an ancestor, and the sweep now passes.
...
Two carry-overs from cycle 1 are unchanged and still the captain's call, neither introduced by this branch: this entity file's own prose still quotes the real numbers and names and is already on `origin/main` (confirmed: 9 lines matching `0928110757`), as is entity 042's `房屋稅` in `apply-insurance-tax-categories.js` and its four fixtures (commit `c1205a8`, already an ancestor of `origin/main`).
