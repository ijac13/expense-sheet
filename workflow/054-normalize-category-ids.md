...
Recommended: fold (2) into this entity — it is a one-line swap to `resolveCategory()` on a surface this migration is guaranteed to break, and AC-12 already implies it. Track (1) as its own entity to be scheduled before or alongside the live run, since a durable fix touches the default-category logic, both subscription forms, and arguably API-side validation, and is a larger change than a data migration should carry.
