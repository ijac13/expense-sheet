...
- DONE: Re-run the full test suite fresh (npm install/ci, not symlinks) and confirm 104/104 in app, with subscription-dates.render.test.js's 26 tests unmodified and passing
  `rm -rf .test-build .test-build-ui && npm ci` (exit 0; a real tree — the only symlinks are the 10 standard `.bin` shims) → **104/104**. `git diff main...HEAD -- app/test/subscription-dates.render.test.js` is 0 lines; run alone it reports 26/26, so the spec's "seven tests" figure is wrong and the build's correction to 26 is right.
- DONE: Independently reproduce at least 3 of the build's mutation claims: the todayLocalIso->toISOString swap (both today-default and cell-id generation), the (year,month)->setMonth swap, and the stopPropagation removal — confirm each fails exactly the tests claimed and no others, then restore and confirm 104/104 again
  All three applied to the `.tsx` and recompiled (tests load `.test-build-ui/`, so a source-only edit would have proved nothing). M1 `todayLocalIso()`→`toISOString()` for today *and* cell ids: exactly 8 fail, including AC-14 (title reads August 2026 instead of September under the `2026-08-31T16:30Z` clock) and AC-15 — matches the claim, and no pre-existing test moved. M3 picker's `e.stopPropagation()` deleted: exactly 1 fails — matches. M2 `(year,month)`→`setMonth`: exactly 1 fails, **not** the three claimed — see discrepancy below. Restored from backup → 104/104, `git status` clean.
- DONE: Investigate the AC-3 finding directly rather than trusting the report's account
  Both host guards confirmed in source, exactly where claimed: `ExpenseEditSheet.tsx:124` and `history/page.tsx:190` each carry `onClick={e => e.stopPropagation()}` on an inner panel sitting inside a wrapper whose own `onClick={onClose}` (`:121`, `:187`). Deleting the picker's guard fails **only** test 32 ("the picker stops its own clicks reaching an UNGUARDED host wrapper") while test 31 — the spec's original end-to-end AC-3 test — still passes, independently reproducing the tautology the build reported. Test 31 survives at `date-picker.render.test.js:261`, still asserts the filter sheet is present, the picker closed, and `onPick` wrote `2026-01-09`, and documents its own caveat inline. Kept, correctly, as regression coverage.
- DONE: Confirm the scope boundary held: reports/page.tsx and subscriptions/page.tsx show zero functional diff … and grep for type="date" across app/app/ matches only the two Subscriptions inputs (AC-26)
  `git diff main...HEAD` is **0 bytes** for `reports/page.tsx`, `reports/DrillDown.tsx`, and `subscriptions/page.tsx`. The sole reports-related change is `app/reports/page.tsx` joining `test:compile` in `package.json` — build tooling enabling AC-21, no runtime behaviour. AC-26: `grep -rn 'type="date"' app/app/` returns exactly 2 hits, both `subscriptions/page.tsx` (`:516`, `:627`).
- DONE: Confirm locale parity: the new picker keys exist in both en and zh common.json with matching key sets, zh genuinely translated not copied, and no aria-label renders as literal English prose
  Flattened both files: 187 keys each, key sets identical, nothing missing either direction. All five `picker.*` keys present in both; zero keys share an identical en/zh value (`上個月`, `下個月`, `選擇年份`, `關閉`, `選擇日期` — correct Traditional Chinese). All four picker `aria-label`s resolve through `t()`; the fifth key `choose_date` is the empty-field placeholder at `ExpenseEditSheet.tsx:231` and `history/page.tsx:249,:260`. No hardcoded English reaches the picker DOM.
- DONE: Live evidence: a plain HTTP check confirming staging/production still serve correctly is sufficient; this entity does not write real data by design
  `curl` → staging `https://expense-sheet-staging.web.app` **HTTP 200**, 11136 B, `<title>Expense Tracker</title>`, real `_next/static/chunks/*` refs; production `https://expense-sheet-b2db8.web.app` **HTTP 200**, 10702 B, same title. `npm run build` on the branch exits 0 and emits the full route table, so the branch is deployable. No writes issued.
- DONE: Mandatory PII/secrets check on the full diff, same bar as every other entity this session
  8 files changed. Across all added lines: zero matches for api-key/secret/password/token/bearer/private-key/`AIza`/`sk-`/`ghp_`/JWT shapes; zero email addresses; zero phone-shaped numbers; zero URLs; zero human-name fixtures. No `.env` file in the diff. Repo hygiene also checked: the root `expense-sheet-b2db8-*.json` service account and `.env.local` are both untracked and gitignored.
- DONE: If everything holds, set verdict PASSED.
  verdict PASSED — all 27 ACs hold. The single discrepancy is in the build report's prose, not in the code or any AC.

### Discrepancy: the setMonth mutation cannot fail AC-6 or AC-8

The build's Falsifiability section claims the `(year,month)`→`setMonth` mutation was "caught by AC-7 … plus AC-6 and AC-8". Reproduced, it is caught by AC-7 alone. The reason is structural, not a matter of mutation shape: the two non-stepping AC-6 cases mount at a fixed value (`2026-03-07`, `2028-02-10`, `2027-02-10`) and never step, so `stepMonth` never runs; the AC-6 column case and AC-8 do step, but from day 7, which exists in every month, so `setMonth` never overflows. No `stepMonth`-scoped mutation can move those four tests.

This is an overstatement in the report, not a defect. The guarantee Decision 5 owes is genuinely pinned: AC-7's 31st-of-January case (`date-picker.render.test.js:418`) fails under `setMonth` (title reads March 2026) and passes under the shipped arithmetic, and it carries its own in-test fixture check that `setMonth` lands in March.

### Summary

Every claim in the build report that I could falsify, I falsified independently, and the substantive ones held: 104/104 from a genuinely clean `npm ci`, the timezone and stacking mutations fail exactly the tests claimed, and the AC-3 tautology finding is real — removing the picker's guard leaves the spec's original end-to-end test green and only the newly added unguarded-host test red. The scope boundary is airtight at 0 bytes of diff across all three out-of-scope files.

One correction to the record: the `setMonth` mutation is caught by AC-7 only, not by AC-6 and AC-8 as the report states — those four tests structurally cannot catch it. That is a reporting overstatement rather than a code or coverage gap, so it does not block.

Worth the captain's attention at the gate: everything here is jsdom plus HTTP, and jsdom computes no layout. The picker's `z-[70]`-over-`z-[60]` stacking, and whether the calendar actually paints above the sheet on a real phone, are the one thing tests cannot confirm — that is the thing to tap through manually on staging.
